...
function Get-StorageInfo {
    ...
    if (-not $disks) { return @([PSCustomObject]@{ Error = 'No SSDs found' }) }
    $isAdmin = ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)
    $result = @()
    foreach ($disk in $disks) {
        $sizeGB = [math]::Round($disk.Size / 1GB, 2)
        $allocGB = [math]::Round($disk.AllocatedSize / 1GB, 2)
        $allocPct = if ($disk.Size -gt 0) { [math]::Round(($disk.AllocatedSize / $disk.Size) * 100, 1) } else { 0 }
        $unallocPct = 100 - $allocPct
        $allocWarning = $unallocPct -ge 5   # warn if more than 5% unallocated
        ...
        if ($isAdmin) {
            ...
            if ($counter) {
                $metrics = [PSCustomObject]@{
                    Temperature    = if ($counter.Temperature) { "$($counter.Temperature) C" } else { 'N/A' }
...
function Get-BatteryInfo {
    ...
    if (-not $bat) { return [PSCustomObject]@{ Error = 'No battery detected' } }
    ...
    if ($healthPct -eq 0) {
        ...
        try {
            ...
            if ($xmlBat -and $xmlBat.DesignCapacity -gt 0) {
                $healthPct = [math]::Round(($xmlBat.FullChargeCapacity / $xmlBat.DesignCapacity) * 100, 0)
...
function Get-BloatwareCheck {
    ...
    $manufacturer = if ($sys.Manufacturer) { $sys.Manufacturer } else { 'Unknown' }
    ...
    if (-not $isHP) {
        ...
        $hpSoftware = Get-ItemProperty -Path $paths -ErrorAction SilentlyContinue |
            ...
            <ScrollViewer Grid.Column="0" VerticalScrollBarVisibility="Auto">
                <StackPanel>
                    ...
                    <Border Style="{StaticResource CardPanel}">
                        <StackPanel>
                            <DockPanel LastChildFill="False">
                                <TextBlock Style="{StaticResource HeaderStyle}" DockPanel.Dock="Left">Primary Battery Health</TextBlock>
                            ...
                            <Grid Margin="0,4,0,0">
                                ...
                                <!-- Row 0: Name (Full width span) -->
                                <StackPanel Grid.Row="0" Grid.Column="0" Grid.ColumnSpan="2" Margin="0,4,0,4">
                                    <TextBlock Style="{StaticResource LabelStyle}">BATTERY NAME &amp; ID</TextBlock>
                                    <TextBlock x:Name="TxtBatteryName" Style="{StaticResource ValueStyle}" TextWrapping="Wrap">-</TextBlock>
                                </StackPanel>

                                <!-- Row 1: Charge & Health -->
                                <Border Grid.Row="1" Grid.Column="0" Margin="0,4,8,4" BorderBrush="#334155" BorderThickness="0,1,0,0" Padding="0,4,0,0">
                                    <StackPanel>
                                        <TextBlock Style="{StaticResource LabelStyle}">CHARGE %</TextBlock>
                                        <TextBlock x:Name="TxtBatteryCharge" Style="{StaticResource ValueStyle}">-</TextBlock>
                                    </StackPanel>
                                </Border>
                                <Border Grid.Row="1" Grid.Column="1" Margin="0,4,0,4" BorderBrush="#334155" BorderThickness="0,1,0,0" Padding="0,4,0,0">
                                    <StackPanel>
                                        <TextBlock Style="{StaticResource LabelStyle}">HEALTH ESTIMATE</TextBlock>
                                        <TextBlock x:Name="TxtBatteryHealth" Style="{StaticResource ValueStyle}">-</TextBlock>
                                    </StackPanel>
                                </Border>

                                <!-- Row 2: Design Voltage -->
                                <Border Grid.Row="2" Grid.Column="0" Grid.ColumnSpan="2" Margin="0,4,0,4" BorderBrush="#334155" BorderThickness="0,1,0,0" Padding="0,4,0,0">
                                    <StackPanel>
                                        <TextBlock Style="{StaticResource LabelStyle}">DESIGN VOLTAGE</TextBlock>
                                        <TextBlock x:Name="TxtBatteryVoltage" Style="{StaticResource ValueStyle}" TextWrapping="Wrap">-</TextBlock>
                                    </StackPanel>
                                </Border>
                            </Grid>
                    ...
                    <Border Style="{StaticResource CardPanel}">
                        <StackPanel>
                            ...
                            <StackPanel Margin="0,8,0,0">
                                <TextBlock Style="{StaticResource LabelStyle}">WINDOWS UTILITY METRICS</TextBlock>
                                <StackPanel x:Name="StkSoftwareReports"/>
                            </StackPanel>
                        </StackPanel>
                    </Border>
                </StackPanel>
...
'BtnUpdateSys', 'TxtCPUName', 'TxtCPUCores', 'TxtCPULogical', 'TxtCPUTemp', 'BtnUpdateCPU',
'TxtBatteryName', 'TxtBatteryCharge', 'TxtBatteryHealth', 'TxtBatteryVoltage', 'TxtBatteryAlert', 'BtnUpdateBattery',
...
'TxtBloatwareDetails', 'BtnUpdateSoftware', 'StkSoftwareReports', 'BtnRunAll', 'BtnClose' | ForEach-Object {
...
$updateBattery = {
    ...
    try {
        ...
        if ($batt.Error) {
            $ui['TxtBatteryName'].Text = "No Battery Found"
            $ui['TxtBatteryCharge'].Text = "N/A"
            $ui['TxtBatteryHealth'].Text = "N/A"
            $ui['TxtBatteryVoltage'].Text = "N/A"
            $ui['TxtBatteryAlert'].Visibility = [System.Windows.Visibility]::Collapsed
        } else {
            $ui['TxtBatteryName'].Text = "$($batt.Name) ($($batt.DeviceID))"
            $ui['TxtBatteryCharge'].Text = "$($batt.ChargePercent)%"
            $ui['TxtBatteryHealth'].Text = "$($batt.HealthPercent)% (Source: $($batt.HealthSource))"
            $ui['TxtBatteryVoltage'].Text = if ($batt.DesignVoltage) { "$($batt.DesignVoltage) V" } else { "N/A" }
            
            if ($batt.VoltageStatus -and $batt.VoltageStatus.IsHigh) {
        ...
        $ui['TxtBatteryName'].Text = "Error loading battery report"
    }
    $ui['TxtStatus'].Text = "READY"
}
...
# 5. GPU adapters
$updateGPU = {
    $ui['TxtStatus'].Text = "SURFACING DISCRETE CORES..."
    [System.Windows.Forms.Application]::DoEvents()
    try {
        $gpus = Get-GPUInfo
        ...
        foreach ($gpu in $gpus) {
            ...
            $tagText.Text = if ($gpu.Source -eq 'SMI') { "NVIDIA-SMI" } else { "WMI/CIM" }
            ...
            $cardStack.Children.Add($driverWrap) | Out-Null
            
            # Temp (instead of nvidia-smi Thermal Node) placed in the same style under Driver Version
            if ($gpu.TemperatureC -gt 0 -and $null -ne $gpu.TemperatureC) {
                ...
                $cardStack.Children.Add($tempWrap) | Out-Null
            }
...
$updateStorage = {
    ...
    try {
        ...
        foreach ($disk in $disks) {
            ...
            $healthText = New-Object System.Windows.Controls.TextBlock
            $healthText.Text = "Status: $($disk.OperationalStatus) [Health: $($disk.HealthStatus)]"
            $healthText.FontSize = 10
            $healthText.TextWrapping = "Wrap"
            $healthText.Foreground = if ($disk.HealthStatus -ne 'Healthy') { [System.Windows.Media.Brushes]::Orange } else { [System.Windows.Media.Brushes]::LimeGreen }
            $stack.Children.Add($healthText) | Out-Null
            
            $alloc = New-Object System.Windows.Controls.TextBlock
            $alloc.Text = "Provisioned Storage: $($disk.SizeGB) GB total, unallocated partition: $($disk.UnallocatedPercent)%"
            $alloc.FontSize = 10
            ...
            if ($disk.Metrics) {
                $met = New-Object System.Windows.Controls.TextBlock
                $met.Text = "SSD metrics: Temp $($disk.Metrics.Temperature) | Power Hours: $($disk.Metrics.PowerOnHours) | Life wear: $($disk.Metrics.Wear)"
                $met.FontSize = 9
                $met.TextWrapping = "Wrap"
                $met.Foreground = Get-Brush "#64748B"
                $stack.Children.Add($met) | Out-Null
            }
...
$updateSecurity = {
    ...
    try {
        ...
        if ($bl.Status -eq 'Admin rights required') {
            ...
            $ui['TxtBitLocker'].Text = "C:\ No Encryption Volume Detected"
            $ui['TxtBitLocker'].Foreground = [System.Windows.Media.Brushes]::Red
        }
...
$updateSoftware = {
    ...
    try {
        ...
        $reports = Get-SoftwareHealthReport
        ...
        foreach ($r in $reports) {
            ...
            $status.Foreground = if ($r.Status -eq 'OK') { [System.Windows.Media.Brushes]::LimeGreen } else { [System.Windows.Media.Brushes]::Orange }
            [System.Windows.Controls.Grid]::SetColumn($status, 1)
            $grid.Children.Add($status) | Out-Null
            
            $detail = New-Object System.Windows.Controls.TextBlock
            $detail.Text = $r.Details + " (Found: $($r.Total))"
            $detail.FontSize = 10
...
# ========== INITAL START LOADING (EVENT DRIVEN FIRST TICK) ==========
